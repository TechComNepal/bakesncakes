<x-cms-employee-master-layout :pageTitle="$pageTitle">
    ...
    <x-form-base :route="'employee.brands.update'" :requiredParam="$brand" :method="'PUT'" :title="$pageTitle"
        :subTitle="$subTitle">
            ...
            <div class="mb-3 @error('category_id') is-invalid @enderror">
                ...
                <select class="form-control js-choices" name="category_id[]" id="categories"
                    ...
                    @foreach ($categories as $key => $category)
                        <option value="{{ $category->id }}" @if (in_array($category->id, old('category_id', $brand->categories->pluck('id')->toArray()), true)) selected @endif>{{ $category->name }}</option>
                    @endforeach
                </select>
            </div>
            @error('category_id')
